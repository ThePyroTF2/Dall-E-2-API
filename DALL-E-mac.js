...
const imageGen = async () => {
  ...
  let filePathRealName = isDupe
    ? await getUndupedFilePath(filePathPromptName)
    : filePathPromptName;

  await fs.promises
    .writeFile(filePathRealName, buffer)
    .then(console.log(`Image saved to ${filePathRealName}`));

  // Add image URL and information to images.json
  let imageURL = `https://raw.githubusercontent.com/justDeeevin/DALL-E-2-API/master/images/${encodeURIComponent(
    path.basename(filePathRealName),
  )}`;
